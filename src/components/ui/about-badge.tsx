...
export default function AboutBadge({
  ...
  // If href is provided, use Link and a span/button inside for accessibility and animation
  if (href) {
    return (
      <Link href={href} className={cn("group", "cursor-pointer")}>
        ...
        >
          ...
          <span>{text}</span>
          {endIcon && (
            <motion.span className="text-foreground/60">{endIcon}</motion.span>
          )}
        </motion.span>
      </Link>
    );
  ...
  // Otherwise, use a button or span (if no click handler)
  const Wrapper = onClick ? motion.button : motion.span;
  return (
    <Wrapper
      type={onClick ? "button" : undefined}
      onClick={onClick}
      className={cn("group", !onClick && "cursor-default")}
      initial={{ opacity: 0, y: -20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8, ease }}
      onHoverStart={() => controls.start("hover")}
